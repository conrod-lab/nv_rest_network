...
# Test functions
def test_within_network_changes():
    num_rois = 300
    ...
    # Simulate increased within-network connectivity in Network 1
    for i in network1_indices:
        for j in network1_indices:
            if i != j:
                correlation_matrix_t2[i, j] = correlation_matrix_t2[j, i] = 0.9
    ...
    print("Available networks:", within_network_changes.keys())
    
    # Use an actual network label from the atlas
    example_network = list(within_network_changes.keys())[0]
    assert within_network_changes[example_network] > 0, f"Within-network connectivity in {example_network} should have increased."

def test_between_network_changes():
    num_rois = 300
    correlation_matrix_t1 = generate_random_correlation_matrix(num_rois)
    correlation_matrix_t2 = generate_random_correlation_matrix(num_rois)
    ...
    # Simulate decreased between-network connectivity between Network 1 and Network 2
    for i in network1_indices:
        for j in network2_indices:
            correlation_matrix_t2[i, j] = correlation_matrix_t2[j, i] = 0.1
    ...
    print("Available network pairs:", between_network_changes.keys())
    
    # Use actual network labels from the atlas
    example_network_pair = list(between_network_changes.keys())[0]
    assert between_network_changes[example_network_pair] < 0, f"Between-network connectivity between {example_network_pair} should have decreased."

# Run tests
test_within_network_changes()
test_between_network_changes()

# Main analysis and plotting code (unchanged)
